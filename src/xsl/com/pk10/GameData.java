...
public class GameData {
	...
	private static Map<String, Object> PK10Map = new HashMap<String, Object>();
	private static List<Map<String,Object>> mail=new ArrayList<Map<String,Object>>();
	

	public static Map<String, Object> getPK10Map() {
		return PK10Map;
	}
	public static void setPK10Map(Map<String, Object> pK10Map) {
		PK10Map = pK10Map;
	}
	public static void GameData() throws UnsupportedEncodingException {
		...
		try {
			...
			if(str.contains("您的账号")){
				...
				PK10Map.put("win_num", FunUtil.StringToInt2(map.get("wins").toString()));
				PK10Map.put("mymoney",Integer.parseInt(map.get("mymoney").toString()));
				PK10Map.put("now_num",map.get("readyid").toString());
				PK10Map.put("readyid", map.get("readyid").toString());
				PK10Map.put("result", data.get("WinNumber").toString().split("\\.")[0]);
				PK10Map.put("resultStr", data.get("WinNumber").toString().split("\\.")[0]);
    ...
    public static boolean single_win(String str){
		...
		try{
			String[] betting=PK10Map.get(str).toString().split(",");
			String rs=PK10Map.get("result").toString();
