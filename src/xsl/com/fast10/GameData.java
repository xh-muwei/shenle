...
public class GameData {
	...
	private static Map<String, Object> FastTenMap = new HashMap<String, Object>();
	...
	public static void GameData() throws UnsupportedEncodingException {
		...
		try {
			...
			if(str.contains("您的账号")){
				...
				FastTenMap.put("win_num", map.get("todayProfit").toString().split("\\.")[0]);
				FastTenMap.put("play_num", map.get("todayBuyNum").toString().split("\\.")[0]);
				FastTenMap.put("now_num",map.get("curLotteryNo").toString());
				FastTenMap.put("lastNum",map.get("lastLotteryNo").toString());
				FastTenMap.put("result",map.get("prizeCode").toString().split("\\.")[0]);	
				WinInfo();
	...
	public static List<Integer> touListMix(Map<String,Integer> map){
	...
	public static List<Integer> touListMax(Map<String,Integer> map){
		 List<Integer> rlist=new ArrayList<>();
		 // 根据map的value排序
		List<Entry<String,Integer>> list=new ArrayList<Entry<String,Integer>>(map.entrySet());
		Collections.sort(list,new Comparator<Map.Entry<String, Integer>>() {
			...
			if(str.contains("您的账号")){
				...
				if(page==1){
					for(int i=3;i<list.size();i++){
						...
						rlist.add(Integer.valueOf(map2.get("prizeCode").toString()));
					}
				}else{
    ...
    public static boolean single_win(String str){
		...
		try{
			String[] betting=FastTenMap.get(str).toString().split(",");
			String rs=FastTenMap.get("result").toString().split(",")[0];
	...
	public static Map<String, Object> getFastTenMap() {
		return FastTenMap;
	}
	public static void setFastTenMap(Map<String, Object> fastTenMap) {
		FastTenMap = fastTenMap;
	}
